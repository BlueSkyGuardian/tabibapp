...
import { accessedDynamicData, createPostponedAbortSignal, formatDynamicAPIAccesses, isPrerenderInterruptedError, createDynamicTrackingState, createDynamicValidationState, trackAllowedDynamicAccess, throwIfDisallowedDynamic, PreludeState, consumeDynamicAccess } from './dynamic-rendering';
                ...
                onError: (err, errorInfo)=>{
                    if (isPrerenderInterruptedError(err) || finalClientController.signal.aborted) {
                        ...
                        if (typeof componentStack === 'string') {
                            trackAllowedDynamicAccess(workStore.route, componentStack, dynamicValidation, clientDynamicTracking);
                        }
                        return;
                    }
                    if (isReactLargeShellError(err)) {
    ...
    try {
       ...
       */ // Prerender controller represents the lifetime of the prerender.
            ...
            if (serverIsDynamic || clientIsDynamic) {
                if (postponed != null) {
                    // Dynamic HTML case
                    metadata.postponed = await getDynamicHTMLPostponedState(postponed, fallbackRouteParams, resumeDataCache);
                } else {
                    // Dynamic Data case
